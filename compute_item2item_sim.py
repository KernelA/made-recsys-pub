...
@hydra.main(config_path="configs", config_name="item2item", version_base="1.3")
def main(config):
    ...
    recommendations = algo.recommend(test_histories, num_recs_per_user, num_recs_per_user * 2)
    ...
    save_json(metric_dir / "map.json",
              {"map": metrics.filter(pl.col("name") == "MAP").get_column("value").max()})
